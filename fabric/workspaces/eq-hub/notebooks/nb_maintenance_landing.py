# Notebook: nb_maintenance_landing
# Layer:    Landing (lh_landing)
# Purpose:  Run OPTIMIZE and VACUUM (4-hour retention) on every Delta table
#           across all schemas in lh_landing. Schemas and tables are discovered
...
spark = SparkSession.builder.appName("nb_maintenance_landing").getOrCreate()
...
if failed:
    ...
    raise RuntimeError(
        f"nb_maintenance_landing completed with {len(failed)} failure(s). "
