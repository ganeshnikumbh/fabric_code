...
# Notebook: nb_gold_dim_email_campaign
# Layer:    Gold
# Purpose:  Full-refresh load of gold.dim_email_campaign from
...
spark = SparkSession.builder.appName("nb_gold_dim_email_campaign").getOrCreate()

_notebook_start = time.time()



_target_table = "lh_gold.gold.dim_email_campaign"
_business_key_cols = ['source_campaign_id','campaign_name','campaign_utm']   # list
_is_scd2           = True
_surrogate_key_col = "email_campaign_key"
_hash_col          = "md5_hash"

print("=" * 65)
print("  nb_gold_dim_email_campaign — START")
print("=" * 65)
...
_unknown_df = spark.range(1).select(
    F.lit(-1)           .cast("long")      .alias("email_campaign_key"),
...
(
    ...
    .merge(_unknown_df.alias("src"), "tgt.email_campaign_key = src.email_campaign_key")
    .whenNotMatchedInsertAll()
    .execute()
)
print(f"  Unknown row (email_campaign_key=-1) ensured in '{_target_table}'")
