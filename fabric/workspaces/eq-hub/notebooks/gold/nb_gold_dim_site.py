...
# Notebook: nb_gold_dim_site
# Layer:    Gold
# Purpose:  SCD Type-1 load of gold.dim_site from lh_silver.silver_s2.csr_base
#           and lh_silver.silver_s2.clr_base (Webex Contact Center domain).
#           Sites represent physical/logical call centre locations (e.g. WDM).
#
# Write pattern: SCD Type-1 via GoldLoader (last-write-wins on site attributes).
#
# Pre-requisites:
...
spark = SparkSession.builder.appName("nb_gold_dim_site").getOrCreate()

_notebook_start = time.time()

_target_table      = "lh_gold.gold.dim_site"
_business_key_cols = ["site_id", "site_name"]
_is_scd2           = False
_surrogate_key_col = "site_key"
_hash_col          = "md5_hash"
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 2 — Read & consolidate site members
# ══════════════════════════════════════════════════════════════════════════════

print(f"\n[1/4] Reading site members from csr_base and clr_base")

site_df = spark.sql(f"""
    SELECT DISTINCT last_site_id  AS site_id,
                    last_site_name AS site_name
    FROM   {_SRC_CSR}
    WHERE  last_site_id IS NOT NULL
...
source_count = site_df.count()
print(f"  Distinct site members : {source_count:,}")
display(site_df.limit(2))
...
# ── Unknown / default row (site_key = -1) ─────────────────────────────────────
from delta.tables import DeltaTable  # noqa: F821  # type: ignore[import]

_unknown_df = spark.range(1).select(
    F.lit(-1)       .cast("long")      .alias("site_key"),
    F.lit("Unknown")                   .alias("site_id"),
    F.lit("Unknown")                   .alias("site_name"),
...
(
    ...
    .merge(_unknown_df.alias("src"), "tgt.site_key = src.site_key")
    .whenNotMatchedInsertAll()
    .execute()
)
print(f"  Unknown row (site_key=-1) ensured in '{_target_table}'")
...
print("\n" + "=" * 65)
print("  nb_gold_dim_site — COMPLETE")
