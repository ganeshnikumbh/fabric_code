...
_landing_ref = f"lh_landing.{p_source_schema}.{p_source_table}" if p_source_schema.strip() else f"lh_landing.{p_source_table}"
...
# ── 3a. ingestion_config ─────────────────────────────────────────────────────
ingestion_config_df = ingestion_config_df_from_json(p_ingestion_config_json)  # noqa: F821  # type: ignore[name-defined]  — injected by %run nb_utils
...
# ── 3b. schema_config ────────────────────────────────────────────────────────
schema_config_df = schema_config_df_from_json(p_schema_config_json)  # noqa: F821  # type: ignore[name-defined]  — injected by %run nb_utils
...
col_map, col_map_lower, hash_cols_ordered = build_col_maps(mappings)  # noqa: F821 — injected by %run nb_utils
...
# If watermark_column is not in schema_config, pass it through as a temp column
# so Section 5 can derive data_date from it. Drop it after.
